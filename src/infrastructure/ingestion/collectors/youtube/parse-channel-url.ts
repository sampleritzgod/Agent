 ...
 *
 * Accepted inputs include, for example:
...
export function parseChannelUrl(rawInput: string): ChannelRef {
  ...
  if (!/^https?:\/\//i.test(input)) {
    ...
    // Fall through by treating it as a URL-less path.
  }
  ...
  try {
    ...
    throw new YouTubeCollectorError(
      "INVALID_CHANNEL_URL",
      `Could not parse channel URL: ${input}`,
      { cause },
    );
  }
  ...
  if (segments.length === 0) {
    throw new YouTubeCollectorError(
      "INVALID_CHANNEL_URL",
      `URL does not point at a channel: ${input}`,
    );
  }
  ...
  switch (first) {
    case "channel":
      if (!second || !CHANNEL_ID_PATTERN.test(second)) {
        throw new YouTubeCollectorError(
          "INVALID_CHANNEL_URL",
          `Invalid channel id in URL: ${input}`,
        );
      }
    ...
    case "c":
      if (!second) {
        throw new YouTubeCollectorError(
          "INVALID_CHANNEL_URL",
          `Missing custom name in URL: ${input}`,
        );
      }
